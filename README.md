...
- **控制桌面（Windows）**：内置远程桌面——实时画面串流 + 鼠标/键盘控制、光标位置与形状实时同步；随葡萄云自动启停，走内部代理呈现
...
│   ├── desktop.js         # 桌面控制托管（拉起子服务、内部代理）
│   ├── desktop-server.cjs # 桌面控制子服务（截屏串流 + 输入注入，CJS）
│   ├── desktop-input.cjs  # 桌面控制底层（Win32 键鼠注入/光标查询，CJS）
│   └── util.js
├── web/                   # 前端（单壳页面，无构建）
│   └── desktop/           # 桌面控制页面资源（由子服务托管）
